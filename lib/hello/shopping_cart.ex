defmodule Hello.ShoppingCart do
  ...
  def add_item_to_cart(cart, product) do
  ...
  def remove_item_from_cart(cart, product) do
    {1, _} =
      Repo.delete_all(
        from(i in CartItem,
          where: i.cart_id == ^cart.id and i.product_id == ^product.id
        )
      )
  end
  ...
  def total_cart_price(cart) do
    Enum.reduce(
      cart.items,
      Decimal.new(0),
      fn item, acc -> Decimal.add(total_item_price(item), acc) end
    )
  end
end
